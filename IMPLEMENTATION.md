...
| 2026-09-09 | v0.12.10 | **Task 层冻结** | **137** | 首次白名单构造 runtime state (base 不从 {...task} spread——防止夹带 claimedAt/forks/eventIndex 等 runtime 字段), Genesis Event 绑定 TaskDefinitionHash (canonicalizeTaskDefinition + v2 canonical 条件包含 + validate 定义一致), publish actor===publisher 强制 (validateTaskEvent+node 双层), Task Definition immutable (重复 publish 不再覆写 title/description), emit 只发 canonical task (this.tasks.get), snapshot 一致性重放 (_reconcileFromEvents——Event Sourcing 原则: events 权威, snapshot 仅缓存跳过不信任), forks 改为 derived view (从 events 重算, 不从 snapshot.forks 信任), P1-⑥/⑦ 测试 |
| 2026-09-09 | v0.12.11 | **Event Sourcing 完成** | **141** | P0-1: fork 事件必须进 Event Log——_handleTaskMessage fork 分支不再 upsert(local) 忽略 event, 改为 upsert(local,event,{fork:true}) (_appendEvent+events[]+eventIndex 写入, canonical lastEventHash/status 不动; _isCanonicalParent 增量推导 branch head), P0-2: eventIndex 全量索引——_tryRebuildFromEvents 重建后把非主链 fork 分支事件补入 eventIndex (height=parent.height+1), 多级 fork D.previousHash=C 不再误判 broken chain; genesis publisher 身份禁止 || packet.author 兜底 (v2 publish 用 event.actor 绝对身份); INVARIANT 1-4 测试 (accepted→log/runtime derivable/fork 重启可恢复/live==replay), reconcile head 选择用 forkRule 政策替代 ts fallback |
